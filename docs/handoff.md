...
9. 에이전트 실행 환경이 정상일 때는 큰 배너를 계속 노출하지 않는다. 헤더 우측 `실행 정상 · 버전` 상태점으로 축약하고, 사용자의 대응이 필요한 비정상 상태만 원인과 행동 버튼이 있는 배너로 유지한다.
10. 실행 준비 자동 계획은 부모 actions 객체가 주기 조회로 새로 만들어져도 반복하지 않는다. 실제 역할·배정 방식·인원·대상·정책·실행 수가 바뀔 때만 다시 확인하며, 최초 계획 완료 뒤 자동 조회로 로딩 상태가 왕복하지 않는다.
...
- workflow-labs 프런트 최신: 28파일·920 passed, typecheck·production build, `git diff --check` 통과.
...
- 최신 debug 앱은 `/Users/catze/project/workflow-labs/src-tauri/target/debug/bundle/macos/LLM Workflow.app`이며 정상 상태가 축약된 에이전트 작업 화면에 열려 있다. 정책 저장과 에이전트 시작은 수행하지 않았다.
