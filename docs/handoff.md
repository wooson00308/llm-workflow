...
6. 에이전트 화면은 정상 상태의 불필요한 갱신 행동, 세 역할 동시 폼과 반복 빈 목록을 정리했다. 개발 보드는 고정 950px 가로 보드와 QA 상태 열을 제거하고 `다음 확인` 한 건과 접힌 대기 큐를 별도 행동 영역으로 둔다. 최신 debug 앱의 1180px 기본 창과 940px 최소 창에서 직접 확인했다.
