# LLM Workflow 제품·기술 기준

작성일: 2026-07-30

## 제품 정의

LLM Workflow는 프로젝트 디렉터리 안의 Markdown 문서를 아이디어, 기획, 승인, 개발 작업, QA 흐름으로 보여주는 로컬 우선 데스크톱 클라이언트다.

앱은 LLM을 실행하거나 제어하지 않는다. Codex, Claude Code 등 외부 LLM이 프로젝트의 Markdown을 읽고 쓰며, 앱은 파일과 메타데이터를 생성·관찰·시각화한다.

```mermaid
flowchart LR
    U["사용자"] --> A["LLM Workflow"]
...
| 외부 LLM | 기획자·프로젝트 아키텍트·개발자 중 한 역할로 아이디어 구체화, 작업 분해 또는 구현·검증 수행 |
...
- 기획 스튜디오: 상태 필터, 문서 목록, Markdown 본문, 결정 패널, 승인·수정 요청·폐기
- 개발 보드: 준비, 진행 중, 막힘, QA 대기, 완료와 카드별 상세 화면
- QA: 상세 화면에서 작업 원문·완료 조건을 읽고 테스트 플로우를 입력해 수정 요청 또는 확인 완료
