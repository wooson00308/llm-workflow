...
mod tests {
    ...
    ///
    /// 앱의 slug 생성은 경로 문자열의 `/`를 `-`로 바꾸고 앞이 `-`가 아니면 `-`를 붙인다. Windows
    /// 경로는 구분자가 `\`이고 드라이브 문자가 앞에 붙으므로 이 규칙이 무엇을 내는지가 여기서 보인다.
    /// 하트비트가 이 값을 프로젝트 루트로 되돌릴 수 있는지는 이 저장소에서 확인할 수 없다 — 그
    /// 역변환은 `/`에서 시작해 존재하는 디렉터리를 최장 일치로 찾아 내려가는 추정이고, 실제 Windows
    /// 환경이 있어야 확인된다(기획서 완료 조건 18, 사용자 QA 항목).
    ///
    /// **규칙을 바꾸지 않는다**(R7). 잡 이름이 하트비트의 상태 키라, 바꾸면 이미 설치된 잡의 실행
    /// 이력과 실행 한도 창이 초기화된다. 이 테스트는 QA에서 역변환이 깨졌을 때 무엇을 바꿔야 하는지를
    /// 한자리에서 보여 주는 용도다.
    #[test]
    fn records_what_the_slug_rule_produces_for_a_windows_shaped_path() {
        assert_eq!(
            ...
            r"-C:\Users\catze\project\workflow-labs"
        );
