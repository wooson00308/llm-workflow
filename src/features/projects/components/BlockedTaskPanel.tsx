...
/**
 * 작업 문서가 작성한 현재 막힘 사유를 그대로 보여 준다.
 *
 * 값의 뜻을 보완하지 않고, 관련 대상도 현재 작업 목록의 id와 정확히 일치할 때만 링크로 만든다.
 * 구조화 절을 읽지 못한 옛 문서는 결정권자 요약 또는 원문 안내로 폴백한다.
 */
export function BlockedTaskPanel({ decisionSummary, onOpenRelatedTask, reason, tasks, updatedAt }: Props) {
  if (reason === null) {
    return (
